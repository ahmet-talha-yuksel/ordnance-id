...
def test_pilot_selection_is_deterministic_and_stratified() -> None:
    eval_set = load_eval_set(Path("evals/datasets/eval_set_v1.yaml"))
    tiers = load_class_tiers(Path("config/class_tiers.yaml")).mapping()
    first = select_pilot(eval_set.samples, Path("data/eval_images"), tiers, seed=0)
    second = select_pilot(eval_set.samples, Path("data/eval_images"), tiers, seed=0)
